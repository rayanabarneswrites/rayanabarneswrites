...
// Create dropdown for mobile/tablet
...
// Create filter buttons for desktop
categories.forEach(cat => {
...
// Insert both filter UI elements
const portfolioSection = document.querySelector('.portfolio');
portfolioSection.insertBefore(filterSelect, portfolioSection.querySelector('.project-grid'));
portfolioSection.insertBefore(filterContainer, portfolioSection.querySelector('.project-grid'));

// Responsive toggle for filter UI
...
window.addEventListener('resize', handleFilterUI);
document.addEventListener('DOMContentLoaded', handleFilterUI);
...
function filterProjects(category) {
    ...
    filtered.forEach(project => {
        ...
        card.innerHTML = `<a href="${project.link}" target="_blank"><h3>${project.title}</h3><p>${project.category}</p></a>`;
